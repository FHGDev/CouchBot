...
namespace MTD.DiscordBot.Modules
{
    ...
    public class YouTube : BaseModule
    {
        ...
        public async Task Owner(string channelId)
        {
            ...
            if (server.ServerYouTubeChannelIds != null && server.ServerYouTubeChannelIds.Contains(channelId))
            {
                await Context.Channel.SendMessageAsync("The channel " + channelId + " is in the list of server YouTube Channels. " +
                    "Please remove it with '!cb youtube remove " + channelId + "' and then retry setting your owner channel.");
