    ...
    public class YouTubeDal : IYouTubeDal
    {
        ...
        public async Task<LiveChatStatusResponse> GetLiveChannels(string channelIds)
        {
            try
            {
                ...
                var webRequest = (HttpWebRequest)WebRequest.Create("https://www.googleapis.com/youtube/v3/liveChat/status?channelId=" + channelIds + "&part=snippet&key=" + _botSettings.KeySettings.YouTubeApiKey + "&fields=items(snippet(channelId,currentVideoId))");
                //webRequest.ContentType = "application/json; charset=utf-8";
